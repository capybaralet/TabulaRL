...
def estimate_return(mu, pi, Q, states, actions, sigma_fn, debug=0):
    ...
    for tstep in range(backup_length)[::-1]: # update G by one time-step
...
#for prob_tree in [0]:
    if False and prob_tree == 0:
        ...
        sigma_fn = lambda x: (1 - prob_tree)#rng.rand() > prob_tree
    ...
    for trial in range(num_trials):
        ...
        print '\n'
        print "prob_tree, trial", prob_tree, trial
        print '\n'
        lr = orig_lr
        ...
        for episode in range(num_episodes):
            ...
            # (off-policy) regret for this episode
            regret = len(states) - 2 * grid_width + 1
            print episode, regret
            mu_returns.append(regret)
