...
#parser.add_argument('--environment', type=str, default='grid_world')
...
for pp, sig in enumerate(sigmas):
    ...
    for trial in range(num_trials):
        ...
        for episode in range(num_episodes):
            ...
            if environment == 'random_walk':
                perfs[pp,trial,episode] = (np.mean((Q - ref_Q)**2))**.5
            else:
                pass
                #perfs[pp,trial,episode] = returns

            np.save('HW1_perfs_' + environment + '_' + sigma + '.npy', perfs)

        # ---------- END ---------- #

np.save('HW1_perfs_' + environment + '_' + sigma + '___COMPLETE', perfs)
